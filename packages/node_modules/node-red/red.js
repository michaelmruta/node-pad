...
process.env.NODE_RED_BUNDLE = process.argv.slice()[2] || process.cwd()
process.env.NODE_RED_HOME = process.argv.slice()[3] || "."
process.env.NODE_RED_THEME = process.argv.slice()[4] || "light"
...
// Recover old workflows
try {
    if( fs.existsSync( process.env.NODE_RED_HOME + "/var" ) ) {
        console.log("Recovering old workflows...");

        const Application = path.join(process.env.NODE_RED_HOME, 
                                      "var","mobile","Containers","Data","Application");
        var hashedFolder = null
        hashedFolder = fs.readdirSync(Application)
        hashedFolder = hashedFolder[hashedFolder.length - 1]    

        const oldFlow = path.join(Application, hashedFolder, "Documents");
        const currentFlow = path.join(process.env.NODE_RED_HOME, 'flows_'+require('os').hostname()+'.json')

        fs.copyFile( oldFlow, currentFlow, (err) => { 
            if (err) { 
               console.log("Error Found:", err); 
            } else {
               fs.unlinkSync(oldFlow);
            }
        }); 
...
if (parsedArgs.settings) {
    ...
    if (fs.existsSync(path.join(process.env.NODE_RED_HOME,".config.json"))) {
        ...
        var userDir = path.join(process.env.NODE_RED_HOME, ".node-red")
...
try {
    settingsFile = path.join(process.env.NODE_RED_HOME, ".node-red","settings.js")    
    if(process.env.NODE_RED_HOME == ".") {
        settingsFile = process.cwd() + "/.node-red/settings.js";
    }    
    var settings = require(settingsFile);
    settings.settingsFile = settingsFile;
    
    var theme_path = path.join(__dirname.replace('/packages/node_modules/node-red',''), 
                               "/node_modules/@node-red-contrib-themes/midnight-red/theme.css");
    settings.editorTheme.page = process.env.NODE_RED_THEME == 'dark' ? { css: theme_path } : {}
} catch(err) {
...
try {
    // login
    var adminAuth = fs.readJsonSync(path.join(process.env.NODE_RED_HOME,"adminAuth.json"));
    if(typeof adminAuth === 'object') {
        if(adminAuth.encrypted == false) {
            for(var u of adminAuth.users) {
                u.password = bcrypt.hashSync(u.password, 8);
            }
        }
        settings.adminAuth = adminAuth;
        console.log("adminAuth.json loaded...")
        delete settings.uiHost;
    }
} catch(e) {
    if(e.code != "ENOENT") {
        console.log(e)
    ...
    RED.start().then(function() {
        if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
            ...
            server.listen(settings.uiPort,settings.uiHost,function() {
                ...
                setInterval(function() {
                    try {
                        var messages = path.join(process.env.NODE_RED_HOME,".system");
                        var n = fs.readFileSync(messages, 'utf8');
                        if (n == "RESUME") {
                            server.close()
