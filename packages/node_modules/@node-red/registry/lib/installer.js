...
function installModule(module,version,url) {
    ...
    activePromise = activePromise.then(() => {
        ...
        return new Promise((resolve,reject) => {
            ...
            log.info("Installing: " + path.join( process.env.NODE_RED_HOME));

            var options = {
                name: module,
                version: version,
                path: path.join( process.env.NODE_RED_HOME),
                forceInstall: false,
                npmLoad: {loglevel:'silent',
                          progress: false,
                          cache:path.join( process.env.NODE_RED_HOME, '.npm')}
            };
            ...
            npmi(options, function (err, result) {
                ...
                // try {
                //     log.info(options.name+'@'+options.version+' installed successfully in '+path.resolve(options.path));
                //     copyRecursiveSync( path.join( process.env.NODE_RED_HOME,'node_modules',module),
                //                        path.join( process.env.NODE_RED_HOME,'nodes',module) )
                // } catch(e) { 
                //     if(e == "EEXIST") {
                //         log.error("Module already loaded.")
                //     } else {
                //         log.error("Module already installed. If this module does not appear in the list of nodes, try restarting the application.")
                //     }
                // }

                // events.emit("runtime-event",{id:"restart-required",payload:{type:"warning",text:"notification.warnings.restartRequired"},retain:true});
                // resolve(registry.setModulePendingUpdated(module,version))
                try {
                    log.info(log._("server.install.installed",{name:module}));
                    resolve(require("./index").addModule(module).then(reportAddedModules));
                } catch(e) {
...
function uninstallModule(module) {
    activePromise = activePromise.then(() => {
        return new Promise((resolve,reject) => {
            ...
            log.info("Uninstalling: " + path.join( process.env.NODE_RED_HOME));
            npm.load({loglevel:'silent',
                ...
                function(err_, result_) {
                    ...
                    npm.commands.uninstall(
                        ...
                        try {
                            fs.removeSync( path.join(process.env.NODE_RED_HOME, 
                                                    "node_modules", 
                                                    module) );
                            log.info(log._("server.install.uninstalled",{name:module}));
                            reportRemovedModules(list);
                            ...
                            log.error(log._("server.install.uninstall-failed",{name:module}));
                            reject(new Error(log._("server.install.uninstall-failed",{name:module})));
                        }
                    });
                }
